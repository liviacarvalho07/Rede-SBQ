CREATE TABLE GRUPO_PESQUISA(
	ID INTEGER PRIMARY KEY AUTO_INCREMENT,
	NOME_GRUPO_PESQUISA VARCHAR(255) NOT NULL 
);

CREATE TABLE ESTADOS(
ID INTEGER PRIMARY KEY AUTO_INCREMENT,
SIGLA VARCHAR(5) NOT NULL,
NOME_ESTADO VARCHAR(255) NOT NULL);

CREATE TABLE CIDADE(
ID INTEGER PRIMARY KEY AUTO_INCREMENT,
NOME_CIDADE VARCHAR(255) NOT NULL,
ESTADO INTEGER NOT NULL,
FOREIGN KEY (ESTADO) REFERENCES ESTADOS(ID));

CREATE TABLE TIPOS_INST(
	ID INTEGER PRIMARY KEY AUTO_INCREMENT,
    NOME_TIPO VARCHAR(30) NOT NULL
);

CREATE TABLE NIVEL_FORMACAO(
	ID INTEGER PRIMARY KEY AUTO_INCREMENT,
    NOME_NIVEL_FORMACAO VARCHAR(50) NOT NULL
);

CREATE TABLE INSTITUICOES_GERAIS(
	ID INTEGER PRIMARY KEY AUTO_INCREMENT,
    NOME_INSTITUICAO VARCHAR(300) NOT NULL,
    TIPO_INSTITUICAO VARCHAR(30) NOT NULL,
    FOREIGN KEY (TIPO_INSTITUICAO) REFERENCES TIPOS_INST(NOME_TIPO)
);

CREATE TABLE cad_pessoal (
    id INTEGER PRIMARY KEY AUTO_INCREMENT,
    nome_pessoal Varchar(255) NOT NULL,
    email_pessoal VARCHAR(255) NOT NULL,
    ESTADO VARCHAR(50) NOT NULL,
    cidade VARCHAR(60) NOT NULL,
    funcao VARCHAR(255) NOT NULL,
    NIVEL_FORMACAO VARCHAR(50) NOT NULL,
    INSTITUICAO_FORMACAO VARCHAR(300) NOT NULL,
    INSTITUICAO_ATUACAO VARCHAR(300) NOT NULL,
    ANO_CONCLUSAO VARCHAR(4) NOT NULL,
    SENHA VARCHAR(255) NOT NULL,
   FOREIGN KEY (INSTITUICAO_FORMACAO) REFERENCES INSTITUICOES_GERAIS( NOME_INSTITUICAO),
	FOREIGN KEY (INSTITUICAO_ATUACAO) REFERENCES INSTITUICOES_GERAIS( NOME_INSTITUICAO),
    FOREIGN KEY (ESTADO) REFERENCES ESTADOS(ID));
);

CREATE TABLE cad_funcional (
    id INTEGER PRIMARY KEY AUTO_INCREMENT,
    nome_funcional VARCHAR(255) NOT NULL,
    email_funcional VARCHAR(255) NOT NULL,
    POSICAO VARCHAR(50) NOT NULL,
    SENHA VARCHAR(255) NOT NULL,
    area_atuacao VARCHAR(255) NOT NULL,
    grupo_pesquisa VARCHAR(255) NOT NULL,
    nome_coordenador VARCHAR(255) NOT NULL,
    email_coordenador VARCHAR(255) NOT NULL,
    informacoes_laboratorio VARCHAR(255) NOT NULL,
    instituicao VARCHAR(255) NOT NULL,
	FOREIGN KEY (instituicao) REFERENCES INSTITUICOES_GERAIS( NOME_INSTITUICAO),
	FOREIGN KEY (instituicao) REFERENCES GRUPOS_PESQUISA( NOME_GRUPO),
FOREIGN KEY (grupo_pesquisa) REFERENCES GRUPO_PESQUISA(NOME_GRUPO_PESQUISA));
);

CREATE TABLE EQUIPAMENTOS_CATALOGO(
    ID INTEGER PRIMARY KEY AUTO_INCREMENT,
    NOME_EQUIPAMENTO VARCHAR(300) NOT NULL,
    INSTITUICAO_PROPRIETARIA VARCHAR(300) NOT NULL,
    MARCA VARCHAR(100) NOT NULL,
    MODELO VARCHAR(100) NOT NULL,
    IMAGEM VARCHAR(500) NOT NULL,
    TIPO_EQUIPAMENTO VARCHAR(50) NOT NULL,
    DESCRICAO VARCHAR(500) NOT NULL,
    #ENDEREÇO
    cep VARCHAR(9) NOT NULL,
    RUA VARCHAR(255) NOT NULL,
    BAIRRO VARCHAR(255) NOT NULL,
    CIDADE VARCHAR(255) NOT NULL,
    #CONTATO
    RESPONSAVEL VARCHAR(255) NOT NULL,
    TELEFONE VARCHAR(13) NOT NULL,
    EMAIL VARCHAR(255) NOT NULL,
    WEBSITE VARCHAR(300) NOT NULL,
    #AREAS DE ATUAÇÃO E ESPECIFICAÇÕES 
    AREA_ATUACAO_UM VARCHAR(255) NOT NULL,
    AREA_ATUACAO_DOIS VARCHAR(255) NOT NULL,
    AREA_ATUACAO_TRES VARCHAR(255) NOT NULL,
    PRINCIPAIS_TECNICAS_UM VARCHAR(255) NOT NULL,
    PRINCIPAIS_TECNICAS_DOIS VARCHAR(255) NOT NULL,
    PRINCIPAIS_TECNICAS_TRES VARCHAR(255) NOT NULL,
    PRINCIPAIS_TECNICAS_QUATRO VARCHAR(255) NOT NULL,
    PRINCIPAIS_TECNICAS_CINCO VARCHAR(255) NOT NULL
	

);
