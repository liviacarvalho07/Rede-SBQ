<?php include "../dependencies/protect.php"; session_start(); ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" type="image/png" href="assets/img/ds.png">
    <title>Rede SBQ</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
    <script src="assets/js/script_slide.js"></script>
    <link rel="stylesheet" href="assets/css/style.css">
    <link rel="stylesheet" href="SobreSBQ/style.css">
</head>
<body>
    <header>
        <nav class="nav-bar">
            <div class="central">
                <div class="logo">
                    <img src="assets/img/logo.jpeg" alt="" width="200px">
                </div>
                    <div class="rede">
                        <h1>REDE SBQ</h1>
                        <div class="usual">
                            <img class="icon" src="assets/img/user.png" alt="">
                            <h3 class="usuario">Usuario:<br> <?php echo $_SESSION["nome"]; ?></h3>
                        </div>
                    </div>
            </div>
            
                <div class="nav-options">
                    <!-- Itens do NavBar -->
                    <div class="nav-itens">
                        <ul>
                            <li class="nav-item"><a href="sobreSBQ.php" class="nav-link">Sobre a Plataforma</a></li>
                            <li class="nav-item"><a href="manual.pdf" class="nav-link">Manual</a></li>
                            <li class="nav-item"><a href="exame.php" class="nav-link">Exame Unificado de Química</a></li>
                            <li class="nav-item"><a href="cadastros_laboratorios.php" class="nav-link">Cadastros</a></li>
                            <li class="nav-item" style="color: red"><a href="../index.php" class="nav-link">Sair da Conta</a></li>
                        </ul>
                    </div>
            </div>
    </header>
    
